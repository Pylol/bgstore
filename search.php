		...
		<ul>
			<?php 								/////////////////书籍种类/////////////////
				for($i=1 ; $i<=7 ; $i++)
				{
					...
					{
						echo "<li><a href='http://125.221.225.210/bgstore/class_list.php?class=$subclass[0]'>". $subclass[2] ."</a></li>";
					}
				}
			?>
		</ul>
	...
	<div id="searchList">
		...
		<?php
			...
			if($class != 0)
			{
				...
				where book_Class in (select subClass_ID from tb_SubClass where mainClass_ID=$class) and book_Name like '%{$key}%'
				LIMIT ".($pagination->page_size * ($page-1)).", ".$pagination->page_size;
			}
			else
			{
